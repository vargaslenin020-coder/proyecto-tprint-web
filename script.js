...
function agregarCarrito(nombre, precio) {
    ...
    actualizarCarrito();
    // Abrir el panel automáticamente al agregar
    document.getElementById("carritoPanel").classList.add("activo");
}

// NUEVA FUNCIÓN: Eliminar un producto específico por su posición
function eliminarDelCarrito(index) {
    carrito.splice(index, 1); // Quita 1 elemento en la posición 'index'
    actualizarCarrito();      // Refresca la lista y el total
}
...
function actualizarCarrito() {
    ...
    carrito.forEach((item, index) => {
        total += item.precio;
        // Ahora cada producto tiene un botón que llama a eliminarDelCarrito(index)
        lista.innerHTML += `
            <li style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; border-bottom: 1px solid #eee; padding-bottom: 8px;">
                <div style="text-align: left;">
                    ...
                    <span style="color: #ff4fa3;">$${item.precio.toFixed(2)}</span>
                </div>
